...
### 1.1 상점 이용 흐름 (MVP)
1. **진입**: 로비 또는 전투 결과 화면에서 추천 진입
2. **재화 확인**: 골드/크리스탈 잔액 표시
3. **상품 선택**: 일일 상점/가챠 상점 중 선택
4. **구매 처리**: 재화 차감 → 아이템 지급
5. **재고/제한 갱신**: 일일 제한은 00:00 리셋
...
### 4.1 구매 실패 처리
- 재화 부족 시 구매 불가 안내
- 에너지 구매 상품은 보유 에너지 상한을 초과하지 않도록 제한
